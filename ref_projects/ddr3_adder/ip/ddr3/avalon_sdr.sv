module avalon_sdr (
    ...
    output  logic [3:0]   avm_m0_byteenable,  // mat na byte chi dung cho ghi
    input   logic         avm_m0_waitrequest, // co san sang
    output  logic [10:0]  avm_m0_burstcount,  // so goi trong 1 giao dich
    // External from avalon control (descriptor register)
    input   logic         do_read,
    input   logic [19:0]  init_addr,
    input   logic [10:0]  burst_length,
    output  logic [31:0] out_data_1, // gia tri ghi vao a
    output  logic [31:0] out_data_2  // gia tri ghi vao b
  );
  ...
  begin
    ...
    case(cur_state)
      ...
      begin
        ...
        else // avm_m0_waitrequest == 0
          next_state = READ_PROC;
      end

      READ_PROC:
      begin
        if (|burst_count) // if burst_count > 0
  ...
  begin
    ...
    case(cur_state)
      ...
      READ_START:
      begin
        avm_m0_address = {init_addr,12'd0}; // from dma to ram
        avm_m0_read = 1'b1; // from dma to ram
    ...
    begin
      case (cur_state)
        ...
        begin
          ...
          begin
            if (burst_count[0])
              out_data_1 <= avm_m0_readdata; //le, gia tri doc tu ram
            else
              out_data_2 <= avm_m0_readdata; //chan, gia tri doc tu ram
          end
        end
