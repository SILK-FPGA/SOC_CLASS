module read_avalon_sdr (
	...
	output  logic         avm_m0_read,  //to ram
	output  logic         avm_m0_write,
	output  logic [255:0] avm_m0_writedata,
	output  logic [31:0]  avm_m0_address, // to ram
	input   logic [255:0] avm_m0_readdata, //from ram
	input   logic         avm_m0_readdatavalid, //from ram
	output  logic [31:0]  avm_m0_byteenable,
	input   logic         avm_m0_waitrequest, //from ram
	output  logic [10:0]  avm_m0_burstcount, // to ram
	// External.
	...
	*/
	...
	assign read_start_rise = !read_start_reg & read_start_i; //pio_out gui 1 
...
//			avm_m0_read        = 1'b0;
	...
	case (state)
		...
		READ_START: begin
			if (!avm_m0_waitrequest) begin // khi tat di nghia la da chap nhan duoc request
				n_state = READ_END;
				n_read = '0;
			end
		end
		...
		READ_END: begin
			if (avm_m0_readdatavalid) begin	 // khi readdatavalid = 1 nghia la da nhan duoc data tu ram (256 bit tu ram)
				if (brcount == 3'd3) begin
					n_state = READ_STOP; // khi brcount = 3 nghia la da nhan duoc 4 lan data tu ram (4*256 bit = 1024 bit)
					n_brcount = '0;
				end else begin
		...
		READ_STOP: begin
			if (count == '0) begin
				...
				n_state = READ_START;
				n_read = 1'b1;
				n_address = address + 32'd256; // tang dia chi de doc tiep theo 256 bit tu ram
				n_count = count + 1'b1;
			end
		end
