module window4x4s2 (
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            dout_valid <= 1'b0; // Mặc định hạ cờ
            
            if (din_valid) begin
                // 1. Quản lý tọa độ (Quét từ trái sang phải, trên xuống dưới)
                ...
                // 2. Đẩy dữ liệu qua Line Buffers (Chờ 32 chu kỳ để rớt xuống hàng dưới)
                row1 <= {row1[30:0], din};
                row2 <= {row2[30:0], row1[31]};
                row3 <= {row3[30:0], row2[31]};

                // 3. Cập nhật cửa sổ 4x4 (Dịch trái, nạp pixel mới vào bên phải)
                ...
                // 4. CHỐT CỜ VALID (STRIDE = 2)
                // Cửa sổ đầy khi row >=3 và col >=3. 
                // Lấy các tọa độ LẺ (3, 5, 7... 31) bằng cách xét bit LSB (bit [0] == 1)
    ...
    // Đảo ngược vị trí bit để Pixel góc trên-trái trôi về LSB, khớp với PyTorch Flatten
