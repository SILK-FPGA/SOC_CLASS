...
static void print_ai_result_word(uint32_t word)
{
    ...
    if (valid && cls < 10u) {
        ...
        printf("|                                                          |\n");
        printf("|               RESULT IS NOT VALID YET                    |\n");
        printf("|                                                          |\n");
        printf("+----------------------------------------------------------+\n");
        printf("|   Result word  : 0x%08X                              |\n", word);
        printf("|   Sequence     : %-3u                                     |\n", seq);
...
static int trigger_and_wait_result(int mem_fd)
{
    ...
    volatile uint32_t *connect_pio =
        (volatile uint32_t *)(bridge_bytes + CONNECT_PIO_OFFSET);
    volatile uint32_t *result_pio =
        (volatile uint32_t *)(bridge_bytes + AI_RESULT_PIO_OFFSET);

    uint32_t old_result = *result_pio;
    unsigned old_seq = (old_result >> 16) & 0xFFu;

    printf("[9] CONNECT address  = 0x%08lX\n",
           H2F_AXI_BASE_PHYS + CONNECT_PIO_OFFSET);
    printf("[9] AI RESULT address = 0x%08lX\n",
           H2F_AXI_BASE_PHYS + AI_RESULT_PIO_OFFSET);
    printf("[9] Previous sequence = %u\n", old_seq);
    ...
    *connect_pio = 1u;
    __sync_synchronize();
    printf("[10] CONNECT HIGH. Waiting for a NEW AI result...\n");
    ...
    *connect_pio = 0u;
    ...
    if (!got_new_result) {
        printf("ERROR: timeout after %u ms waiting for a new AI result\n",
...
int main(int argc, char **argv)
{
    ...
    printf("=== HPS LOAD BNN IMAGE + WAIT FOR FPGA CLASS RESULT ===\n");
    ...
    printf("AI result offset : 0x%08lX\n", AI_RESULT_PIO_OFFSET);
    printf("AI timeout       : %u ms\n", AI_RESULT_TIMEOUT_MS);
