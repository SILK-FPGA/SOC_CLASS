module bnn_avl_wrapper (
    ...
    // ---------------------------------------------------
    // Avalon Memory-Mapped Agent (HPS CSR)
    // ---------------------------------------------------
    input  logic [2:0]  avl_mm_address,   
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (sw_soft_reset) sw_soft_reset <= 1'b0; // Tự động xóa
  ...
  logic        rx_done;
  logic [31:0] debug_total_rx_beats; 

  assign avl_s_ready = !rx_done; 

  // ===================================================
  // 2. MÔ PHỎNG TIMING & ĐIỀU KHIỂN BNN
  // ===================================================
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      shift_reg            <= '0;
      bnn_rst_n            <= 1'b0;
    end else if (sw_soft_reset) begin
      // SOFT RESET TỪ CPU (Synchronous)
      rx_word_idx          <= '0;
      rx_done              <= 1'b0;
      tx_word_idx          <= '0;
      bit_cnt              <= '0;
      ...
      shift_reg            <= '0;
      bnn_rst_n            <= 1'b0;
      // Không clear debug_total_rx_beats để phục vụ phân tích log
    end else begin
      ...
      // --- GIAI ĐOẠN 2: Bơm data vào BNN chuẩn luồng 1024-bit liên tục ---
      if (rx_done && bnn_rst_n) begin 
        if (!is_shifting) begin
          ...
          if (bit_cnt > 1) begin
            ...
            // BẢN VÁ: Load word tiếp theo NGAY LẬP TỨC (Không tạo khoảng hở Pipeline)
            if (tx_word_idx < 6'd32) begin
                shift_reg   <= img_buf[tx_word_idx];
                bit_cnt     <= 6'd32;
                // is_shifting vẫn giữ nguyên mức 1
                tx_word_idx <= tx_word_idx + 1'b1;
            end else begin
                is_shifting <= 1'b0; // Chỉ hạ cờ khi đã bơm sạch 1024 bits
            end
          end
        end
      end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (sys_valid) begin
        ...
        done_flag  <= 1'b1; // Chốt cờ W1C
      end else if (avl_mm_write && avl_mm_address == 3'd0 && avl_mm_writedata[31]) begin
        done_flag  <= 1'b0; // CPU tự tay xóa cờ
      end
    end
  end

  always_comb begin
    // Thả rông dữ liệu ra bus bám theo địa chỉ, không kẹp điều kiện avl_mm_read
    case (avl_mm_address)
      ...
      default: avl_mm_readdata = 32'hDEADBEEF;
    endcase
  end
